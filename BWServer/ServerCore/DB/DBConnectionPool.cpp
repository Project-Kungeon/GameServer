...
bool DBConnectionPool::Connect(int32 connectionCount, const WCHAR* connectionString)
{
	...
	{
		spdlog::error("Failed to allocate SQL_HANDLE environment");
		return false;
	}
	...
	{
		spdlog::error("Failed to set SQL_OV_ODBC3");
		return false;
	}
...
void DBConnectionPool::Clear()
{
	...
	spdlog::info("Successfully closed database");
	_connections.clear();
}
...
DBConnection* DBConnectionPool::Pop()
{
	...
	spdlog::trace("Popping connection");
	return conn;
}
...
void DBConnectionPool::Push(DBConnection* connection)
{
	WRITE_LOCK;
	_connections.push_back(connection);
	spdlog::trace("Pushing connection");
}
